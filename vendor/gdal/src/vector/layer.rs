...
pub trait LayerAccess: Sized {
    ...
    ///
    /// Field names are validated as C strings and retained until the GDAL call
    /// returns. GDAL copies the null-terminated list into the layer state.
    /// Use the special names `OGR_GEOMETRY` and `OGR_STYLE` to ignore those
    /// pseudo-fields when supported by the underlying driver.
