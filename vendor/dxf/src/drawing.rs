...
use crate::code_pair_iter::{
    new_code_pair_iter_from_owned_reader, new_code_pair_iter_from_reader, CodePairIter,
};
...
/// yields model-space entities one logical entity at a time.
///
/// Unlike [`Drawing::load`], this reader does not copy the complete input into
/// a byte vector and does not retain the `ENTITIES` section. `POLYLINE` /
/// `VERTEX`, attributed `INSERT`, and attached `MTEXT` sequences are grouped
/// exactly like the materializing loader before they are yielded.
pub struct DrawingEntityReader {
    drawing: Drawing,
    iter: CodePairPutBack,
    pending_entity: Option<Entity>,
    entities_active: bool,
    complete: bool,
}

impl DrawingEntityReader {
    /// Opens a progressive reader from an owned DXF input.
    pub fn load<T>(reader: T) -> DxfResult<Self>
    where
        T: Read + 'static,
    {
        Self::load_with_encoding(reader, encoding_rs::WINDOWS_1252)
    }

    /// Opens a progressive reader from an owned DXF input with explicit text
    /// encoding.
    pub fn load_with_encoding<T>(reader: T, encoding: &'static Encoding) -> DxfResult<Self>
    where
        T: Read + 'static,
    {
        let iter = new_code_pair_iter_from_owned_reader(reader, encoding)?;
        Self::from_iter(iter)
    }

    /// Opens a progressive reader from a file using a buffered input.
    pub fn load_file(path: impl AsRef<Path>) -> DxfResult<Self> {
        Self::load_file_with_encoding(path, encoding_rs::WINDOWS_1252)
    }

    /// Opens a progressive reader from a file with explicit text encoding.
    pub fn load_file_with_encoding(
        path: impl AsRef<Path>,
        encoding: &'static Encoding,
    ) -> DxfResult<Self> {
        let file = File::open(path)?;
        Self::load_with_encoding(BufReader::new(file), encoding)
    }

    fn from_iter(iter: Box<dyn CodePairIter>) -> DxfResult<Self> {
        let mut drawing = Drawing::new();
        drawing.clear();
        let mut iter = CodePairPutBack::from_code_pair_iter(iter);
        let entities_active = Self::read_until_entities(&mut drawing, &mut iter)?;
        Ok(Self {
            drawing,
            iter,
            pending_entity: None,
            entities_active,
            complete: !entities_active,
        })
    }

    /// Metadata parsed before `ENTITIES`, including block definitions needed
    /// to expand `INSERT` entities.
    pub fn drawing(&self) -> &Drawing {
        &self.drawing
    }

    /// Returns the next logical model-space entity.
    pub fn next_entity(&mut self) -> DxfResult<Option<Entity>> {
        if !self.entities_active {
            return Ok(None);
        }
        let Some(entity) = self.read_raw_entity()? else {
        ...
        let entity = match entity {
            Entity {
                common,
                specific: EntityType::Attribute(mut attribute),
            } => {
                if let Some(next) = self.read_raw_entity()? {
                    match next {
                        Entity {
                            specific: EntityType::MText(m_text),
                            ..
                        } => attribute.m_text = m_text,
                        other => self.pending_entity = Some(other),
                    }
                }
                Entity {
                    common,
                    specific: EntityType::Attribute(attribute),
                }
            }
            Entity {
                common,
                specific: EntityType::AttributeDefinition(mut attribute),
            } => {
                if let Some(next) = self.read_raw_entity()? {
                    match next {
                        Entity {
                ...
                Entity {
                    common,
                    specific: EntityType::AttributeDefinition(attribute),
                }
            }
            Entity {
                common,
                specific: EntityType::Insert(mut insert),
            } if insert.__has_attributes => {
                loop {
                    match self.read_raw_entity()? {
                        Some(Entity {
                            specific: EntityType::Attribute(attribute),
                            ..
                        }) => insert
                            .__attributes_and_handles
                            .push((attribute, AUTO_REPLACE_HANDLE)),
                        Some(other) => {
                            self.pending_entity = Some(other);
                            break;
                        }
                        None => break,
                    }
            ...
            Entity {
                common,
                specific: EntityType::Polyline(mut polyline),
            } => {
                loop {
                    match self.read_raw_entity()? {
                        Some(Entity {
                ...
                Entity {
                    common,
                    specific: EntityType::Polyline(polyline),
                }
            }
            other => other,
        };
    ...
    /// Drains any remaining entities and returns complete drawing metadata,
    /// including sections that follow `ENTITIES` such as `OBJECTS`.
    pub fn finish(mut self) -> DxfResult<Drawing> {
    ...
    fn read_raw_entity(&mut self) -> DxfResult<Option<Entity>> {
    ...
    fn swallow_seqend(&mut self) -> DxfResult<()> {
        if let Some(entity) = self.read_raw_entity()? {
            if !matches!(entity.specific, EntityType::Seqend(_)) {
    ...
    fn finish_entities(&mut self) -> DxfResult<()> {
        match self.iter.next() {
            Some(Ok(CodePair {
                code: 0,
                value: CodePairValue::Str(ref value),
                ..
            })) if value == "ENDSEC" => {}
            Some(Ok(pair)) => {
        ...
        Drawing::read_sections(&mut self.drawing, &mut self.iter)?;
        match self.iter.next() {
            Some(Ok(CodePair {
                code: 0,
                value: CodePairValue::Str(ref value),
                ...
                Some(Ok(pair @ CodePair { code: 0, .. })) => match &*pair.assert_string()? {
                    ...
                    "SECTION" => {
                        let section = match iter.next() {
                            Some(Ok(CodePair {
                                code: 2,
                                value: CodePairValue::Str(section),
                                ..
                            })) => section,
                            Some(Ok(pair)) => {
                                return Err(DxfError::UnexpectedCodePair(
...
mod progressive_reader_tests {
    ...
    #[test]
    fn progressive_reader_does_not_read_to_end_before_first_entity() {
        ...
        let mut reader = DrawingEntityReader::load(input).unwrap();
        assert!(bytes_read.load(Ordering::Relaxed) < total / 10);
        assert!(matches!(
            reader.next_entity().unwrap().unwrap().specific,
            EntityType::Line(_)
        ));
        assert!(bytes_read.load(Ordering::Relaxed) < total / 10);

        let drawing = reader.finish().unwrap();
        assert_eq!(drawing.entities().count(), 0);
        assert_eq!(bytes_read.load(Ordering::Relaxed), total);
    }
}
