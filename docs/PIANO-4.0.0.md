...
| B12c | Il sink IPC dichiara sette dei sedici tipi canonici | `descriptor.rs` `SIMPLE_WKB_GEOMETRY_TYPES`, ARROW-VOCABULARY-1.0 | il driver IPC trasporta WKB **senza interpretarlo**, e dichiara `WKB_EWKB_PASSTHROUGH_GEOMETRY`, che restringe a sette tipi | è quella restrizione, e non il vuoto, a far rifiutare `unresolved`. Per un passaggio di byte la restrizione è più stretta del vero; ma la costante è condivisa con CSV, GeoParquet, GPKG e XLS, dove non lo è altrettanto | **aperto, e la strada a monte è più lunga di quanto pensassi**: la PR #6 non aggiunge più la chiave alla `1.0`, perché quel vocabolario si dichiara chiuso — l'analisi è la decisione 0006 | **il comportamento concreto che manca**, e si divide in due metà indipendenti. *Locale*: `GeometryColumnContract` non ha modo di dire «scandito, nessuna geometria»; `types_declaration` vale `Unresolved` sia quando i tipi non si sono determinati sia quando non ce ne sono. Serve un terzo stato **interno** — o un booleano `scansione_completa` accanto — che il reader GeoJSON valorizzi quando la scansione è arrivata a fine file, e che `validate_write` legga per accettare un contratto senza geometrie verso un sink che restringe i tipi. Non tocca nessun contratto condiviso e si prova su una sorgente vuota che oggi viene rifiutata. *Sul filo*: dirlo a un altro componente richiede il successore del vocabolario, perché `plenora.geometry.*` è chiuso — ed è la decisione 0006 | la sorgente vuota di GeoJSON si consegna, con `types_declaration=unresolved` sul filo e l'assenza accertata nel contratto interno. Il pin resta `453c8d1`, e nessun requisito del verificatore dipende dalla proposta |
...
| B13 | Streaming senza materializzazione completa | ARROW-011 | il lettore è a batch e lo spool è documentato, ma **nessuna superficie pubblica espone uno stream**: il documento capability dichiara per `io.read` il solo `application/vnd.apache.arrow.file`, perché CLI-2.0 §4 riserva stdout alla busta | l'antecedente di ARROW-011 è falso oggi, e il catalogo comune invece ammette `arrow.stream` per `io.read`: c'è una capacità che il catalogo prevede e questa superficie non serve | **il comportamento concreto che manca**, in tre pezzi: (1) una superficie che renda i batch senza materializzare — per la CLI significa un secondo flusso o un descrittore di consegna diverso da un file, per il Rust un iteratore di `RecordBatch` che è già ciò che il core ha; (2) la **proiezione** delle colonne, che è l'ingresso che rende utile lo streaming e che oggi non esiste in nessun binding; (3) la dichiarazione nel descrittore se la materializzazione resta limitata, che ARROW-011 ammette esplicitamente come alternativa | il consumatore elabora il primo batch prima che l'ultimo sia stato prodotto — misurabile sull'istante in cui il primo batch è leggibile contro l'istante in cui la sorgente finisce — **oppure** il descrittore dichiara la materializzazione limitata e una sonda lo verifica |
...
| C1b | Il pacchetto si chiama ancora `plenora-io-cli` | — | il nome dice «CLI» e contiene una libreria | infelice ma non scorretto: SURFACE-BINDINGS-1.0 §2 dice che «this repository does not prescribe Rust module, trait, method or type names» | **aperto, e da risolvere prima di stabilizzare l'API**: non è una prova di non conformità — SURFACE-BINDINGS-1.0 §2 dice che il repository non prescrive nomi — ma una volta che un consumatore dipende da `plenora-io-cli`, rinominare diventa una rottura sua oltre che nostra. Rinominarlo tocca cinquantatré file fra CI, gate, manifesti e il workspace di fuzzing | il nome nuovo compare in un posto solo — il manifesto — e tutto il resto lo deriva |
...
* **ARROW-011** — «An operation advertised with Arrow **stream** output MUST
  ...
  è falso. Diventa vero con B13, e allora il requisito entra nella categoria di
  sopra;
...
* **B13** — lo streaming senza materializzazione completa, e con esso la
  proiezione delle colonne, che è l'ingresso che lo rende utile;
* **C1b** — il nome del pacchetto, da risolvere **prima** che un consumatore ne
  dipenda;
