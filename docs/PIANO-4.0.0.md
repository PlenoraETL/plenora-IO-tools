...
| B13 | Streaming senza materializzazione completa | ARROW-011 | il lettore è a batch, ma **nessuna superficie pubblica espone uno stream**: il documento capability dichiara per `io.read` il solo `application/vnd.apache.arrow.file` | il catalogo comune ammette per `io.read` anche `arrow.stream`: c'è una capacità che il catalogo prevede e questa superficie non serve | **chiusa da una decisione, e la decisione è misurata.** La capacità implementata è `application/vnd.apache.arrow.file`; quella non implementata è `application/vnd.apache.arrow.stream`. La ragione non è che manchi il tempo: tutti e dieci i driver sono `DeliverySemantics::OperationAtomic` — se una violazione emerge in un punto qualsiasi della sorgente, l'operazione è rifiutata come blocco unico e nessun prefisso accettato viene consegnato. Consegna incrementale e atomicità dell'operazione si escludono: annunciare uno stream prometterebbe la prima mentre il prodotto sceglie la seconda. La variante `Streaming` esiste nell'asse ed è dichiarabile, ma vuole una categoria d'errore nuova e un bump del protocollo, nessuno dei due ratificato. Il descrittore lo **dichiara** — `attributes.materialization: bounded`, `delivery: operation_atomic` — come diagnostica opaca per CAP-013, perché la selezione si fa sui content type e non su chiavi che il profilo non tipizza | quattro sonde in `tests/capacita_dichiarate.rs`, che partono dal descrittore e cercano il comportamento invece del contrario: `io.read` annuncia il solo file; il consegnato porta il magic `ARROW1` ai due capi, cioè è davvero il formato file e non uno stream in un file; ogni driver disponibile del catalogo dichiara `operation_atomic`, così che la ragione cada insieme al fatto se un giorno cambiasse; e le sei operazioni dichiarano le due superfici. La **proiezione** delle colonne resta fuori, ed è una capacità sua: non è richiesta da ARROW-011 e non è promessa da nessun descrittore |
