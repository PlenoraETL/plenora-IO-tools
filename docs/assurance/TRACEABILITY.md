...
| PLN-ASR-007 | H-01 perdita dati silenziosa | Conversioni e coercion sono fail-closed o rendicontate | ADR-IO 3 e 5; capability gate; `FidelityAssessment` e `LossReport`; parser dei metadati geometrici chiuso e distinzione assente/`unknown` | Parziale per driver |
...
| PLN-ASR-010 | H-07 baseline non riproducibile | Toolchain e grafo dipendenze sono fissati e sottoposti ad audit | `rust-toolchain.toml` 1.92.0; dipendenze dirette con pin esatti e punto unico; gate manifest; `Cargo.lock`, `--locked`, `cargo audit --deny warnings`; eccezioni motivate in `DEPENDENCY_EXCEPTIONS.md` | Parziale: Actions non pinning SHA |
