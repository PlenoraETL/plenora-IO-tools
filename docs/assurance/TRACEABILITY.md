...
| PLN-ASR-005 | H-02 sovrascrittura output | Publish atomico, same-filesystem e no-clobber autorevole | ADR-IO 2; test cross-filesystem, TOCTOU, crash e recovery | Soddisfatto su Linux/Windows |
...
| PLN-ASR-008 | H-06 interpretazione CRS errata | CRS assente, irrisolto e axis order non sono confusi | ADR-IO 4; `CrsResolution`, `RawCrs`, test axis order | Parziale per resolver |
...
| PLN-ASR-010 | H-07 baseline non riproducibile | Toolchain e grafo dipendenze sono fissati e sottoposti ad audit | Rust 1.92, `Cargo.lock`, `--locked`, `cargo audit --deny warnings`; eccezioni motivate in `DEPENDENCY_EXCEPTIONS.md` | Parziale: Actions non pinning SHA |
| PLN-ASR-011 | H-08 verifica insufficiente | Regressioni hanno test deterministici e coverage misurata | CI Linux/Windows, FileGDB feature-on, LCOV e fuzz target | Parziale: MC/DC assente |
