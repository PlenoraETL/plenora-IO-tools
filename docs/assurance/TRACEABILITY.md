...
| PLN-ASR-003 | H-01 corruzione geometrica | WKB/EWKB troncati, dimensionalmente incoerenti o oltre limite sono rifiutati | Unico codec autoritativo `plenora-io-model::wkb_lossless`, esposto da `wkb`; test differenziali API XY/lossless, core e fuzz; regressione Shape Z/M mancanti | Soddisfatto |
...
| PLN-ASR-009 | H-01/H-02 stato parziale | Lifecycle writer invalida dopo errore e pubblica soltanto a `finish` | ADR-IO 1; staging same-filesystem creato dal core; test poison, abort, concorrenza e crash FileGDB | Soddisfatto nel profilo corrente |
...
| PLN-ASR-013 | H-01 valore inventato | Ogni fallback semantico non-panicking è censito e non può crescere senza review | `FALLBACK_REGISTER.md`; gate a 82 occorrenze `check_assurance_fallbacks.sh`; regressioni CSV/CRS/FileGDB, interi, GeoJSON/KML vuoti e XLSX XY | Parziale: registro semantico, non prova formale |
