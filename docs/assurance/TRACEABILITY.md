...
| PLN-ASR-003 | H-01 corruzione geometrica | WKB/EWKB troncati, dimensionalmente incoerenti o oltre limite sono rifiutati | Unico codec autoritativo `plenora-io-model::wkb_lossless`, esposto da `wkb`; visitor strutturale senza AST confrontato dal fuzz con il decoder su accettazione, tipo, dimensioni e SRID; test differenziali API XY/lossless e regressione Shape Z/M mancanti | Soddisfatto |
...
| PLN-ASR-019 | H-01/H-08 codec divergenti non rilevati | WKB/EWKB usa corpus e invarianti condivisi e classifica ogni divergenza fra i due bordi | `WKB_EWKB_FUZZ_COORDINATION.md`; schema manifest condiviso; revisioni IO/database fissate; smoke locale pregresso 61.860.000 iterazioni e smoke visitor/decoder 19.920.000 iterazioni senza finding; replay di quattro finding GeoJSON corretti | Aperto: protocollo proposto, replay incrociato e campagna condivisa non ancora avviati |
| PLN-ASR-020 | H-01/H-07 risultato non riproducibile | Ogni operazione dichiara il livello di determinismo e il catalogo non dipende dall'ordine di registrazione | `DeterminismLevel`; `read_determinism`/`write_determinism`; catalogo ordinato e marcato `byte_for_byte`; test di serializzazione e doppio round-trip su nove driver pure-Rust | Parziale: backend FileGDB/GDAL e confronto multipiattaforma non ancora verificati |
