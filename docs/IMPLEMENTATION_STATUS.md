...
| ADR-IO 6 — projection e pruning | Parziale avanzato trasversale | Contratto `ReadRequest` e schema effettivo; capability `projection_support` nel descrittore v3; projection esatta GeoParquet/IPC; `Required` fail-closed tipizzato su tutti gli altri driver; matrice runtime pure-Rust; pruning conservativo GeoParquet e GeoPackage tramite `gpkg_rtree_index` registrato/conforme, con capability effettiva nel metadata del layer; `target_bytes` combinato con `max_rows` nei reader incrementali; wrapper comune di slicing per IPC e reader materializzati KML/DXF/XLSX, con ordine, schema e lifecycle preservati | `pruning_predicate` nativo resta specifico di GeoParquet e gli altri formati non hanno ancora pruning; la stima per colonne variabili/geometrie è best-effort e non un limite rigido di memoria; lo slicing Arrow non libera i buffer già materializzati |
...
- Il catalogo espone capability di scrittura, concorrenza reader e supporto
  projection con `descriptor_version = 3`.
