...
| ADR-IO 4 — CRS | Parziale | `CrsResolution::{Resolved, DeclaredButUnresolved, Missing}` e `RawCrs`; CSV/XLSX richiedono `assume_crs`; CRS fissi KML/GeoJSON validati in `create`; GPKG/SHP/GeoParquet/IPC propagano metadati CRS | Estrazione CRS embedded DXF assente; non tutti i parser conservano ancora il CRS grezzo non risolto; la copertura dei casi unresolved e degli ordini assi è incompleta |
| ADR-IO 5 — fedeltà | Parziale | `Fidelity` nel descrittore; `LossReport` bounded; DXF registra approssimazioni e attributi non rappresentati; round-trip presenti sui driver principali | Manca `FidelityAssessment` per dataset/contratto; molti driver conditional restituiscono ancora report vuoti; oracoli indipendenti e corpus reali non sono uniformi |
...
- Il contratto geometrico distingue `Wkb`/`Ewkb`, `Xy`/`Xyz`/`Xym`/`Xyzm`,
  ...
  ambigue rispetto ai tipi `Multi*`; una pre-validazione XML limitata protegge
  ...
  rifiutati perché non rappresentabili dal WKT semplice. Rimane parziale il
  percorso DXF, che proietta esplicitamente sul modello `geo-types` XY.
- `Limits` è parte di `ReadOptions` e `WriteOptions`. La dimensione complessiva
  ...
  publish per file singoli, loose set Shapefile e directory FileGDB.
...
CSV/XLSX. Una campagna breve ha individuato e chiuso l'accettazione di coordinate
