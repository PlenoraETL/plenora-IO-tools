...
| ADR-IO 6 — projection e pruning | Parziale avanzato trasversale | Contratto `ReadRequest` e schema effettivo; descrittore v5 con capability machine-readable per projection, pruning numerico e spaziale, determinismo e tipi geometrici; projection esatta GeoParquet, IPC e GeoPackage, inclusa projection vuota e geometria non forzata per richieste tabellari; `Required` fail-closed sugli altri; `NumericComparison` GeoParquet precision-safe su statistiche min/max, con legacy `Opaque` fail-open; pruning spaziale GeoParquet e GeoPackage tramite RTree registrato/conforme; `target_bytes`/`max_rows` su tutti i reader e correzione adattiva GeoPackage sui byte Arrow osservati | Il pruning attributivo resta disponibile solo dove esistono statistiche a blocchi (GeoParquet): usare indici B-tree degli altri formati sarebbe filtering esatto; sugli altri reader la stima byte resta best-effort e lo slicing Arrow non libera buffer già materializzati |
