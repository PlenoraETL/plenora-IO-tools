...
- Il catalogo espone capability di scrittura, concorrenza reader, projection e
  pruning con descrittori versionati; FileGDB usa `descriptor_version = 6` per
  il profilo di fedeltà M/ZM ristretto e verificato.
...
matrice deterministica `RawCrs`/axis order e il test feature-on FileGDB sono ora
